...
# ----Configuration----  
  fileSysRoot <- "/var/data/rawdiag"
  session$userData$user_token <- UUIDgenerate()
  session$userData$dataDir <- file.path(fileSysRoot, session$userData$user_token)
  dir.create(session$userData$dataDir, recursive=TRUE)
  values <- reactiveValues(pdfcontent=NULL,
                           filesystemRoot=session$userData$dataDir,
                           filesystemDataDir = list.dirs(session$userData$dataDir,full.names=F),
...
# ----tabsetPanel ----
  ...
  getUploadedFiles <- reactive({
    if (is.null(input$rawFileUpload)) return()
    files <- c()
    for(i in 1:length(input$rawFileUpload[,1])){
      filename <- input$rawFileUpload[[i, 'name']]
      target <- file.path(values$filesystemRoot,filename)
      file.copy(input$rawFileUpload[[i, 'datapath']], target )
  ...
  getRawfiles <- reactive({
    message(values$filesystemRoot)
    
    f <- getUploadedFiles()#list.files(values$filesystemRoot, recursive=T)
    f[grep("raw$", f, ignore.case=T)]
...
# ----Source----  
  output$sourceFilesystem <- renderUI({
    if (input$source == 'filesystem'){
      tagList(
        selectInput('root', 'root:', values$filesystemRoot,  multiple = FALSE),
        htmlOutput('rawfile')
        )
    } 
  })
  ...
  rawData <- eventReactive(input$load, {
    ...
    if (input$source == 'filesystem'){
      ...
      rf <- file.path(values$filesystemDataDir, file.path(input$root, input$rawfile))
